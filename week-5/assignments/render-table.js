const htmlToElement = (html) => {
  ...
  const renderTable = () => {
    ...
    if (withFilters) {
      ...
      data = data.filter((obj) => obj.first_name.toLowerCase().includes(search_term) 
    || obj.last_name.toLowerCase().includes(search_term)
    || obj.email.toLowerCase().includes(search_term)
    || obj.gender.toLowerCase().includes(search_term)
    || obj.ip_address.includes(search_term)
      );
    }
